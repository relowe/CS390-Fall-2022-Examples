...
class Parser:
    ...
    def __program(self):
        ...
        while not self.__has(Token.EOF) and not self.__has(Token.ELSE) and not self.__has(Token.END): 
    ...
    def __branch(self):
        ...
        c = self.__condition()
        b1 = self.__program()
        if self.__has(Token.END):
            self.__next()
            result = ParseTree(ParseType.IF, token)
            result.children = [ c, b1 ]
            return result
        elif self.__must_be(Token.ELSE):
            self.__next()
            result = ParseTree(ParseType.IFELSE, token)
            b2 = self.__program()
